...
>
> **Mise à jour du 2026-07-26 : Chantier 6 (« mode réel ») quasi-complet.** L'utilisateur a demandé de sortir du mode démo, puis (même jour) a fixé un objectif permanent via `/goal` : livrer le SaaS 100% fonctionnel sans données de démo (paiement excepté, dernière étape à faire ensemble). Audit complet fait (voir [Chantier 6](#chantier-6--sortir-du-mode-démo-vers-un-vrai-saas)) ; boutons de connexion démo retirés de la production ; tableau de bord principal branché sur de vraies données de bout en bout ; **inscription de nouveaux établissements ajoutée** (le plus gros manque structurel — ce n'était auparavant qu'une app mono-tenant) ; **les 6 modules à backend existant (scholarships, hr, internships, attendance, room-booking, results) branchés** (6.4) ; **les 4 derniers modules sans aucun modèle Prisma (notifications, library, transport, advising) créés de zéro — schéma + migrations + API + frontend** (6.4b), plus deux vrais bugs de faux-succès corrigés dans `import-export-page.tsx` (import qui prétendait réussir sans jamais écrire en base, 4 des 6 exports qui généraient des données inventées). Reste : `RESEND_API_KEY` à fournir, Mobile Money réel (attente utilisateur), et le paiement (dernière étape, à faire avec l'utilisateur). Voir le journal Chantier 6.4b pour le détail complet et les limitations connues (notamment : deux agents parallèles ont été interrompus par la limite de dépense mensuelle du compte pendant cette passe — leur travail a été vérifié et est complet, mais l'utilisateur doit relever cette limite sur claude.ai/settings/usage pour que le rythme de travail autonome du `/goal` puisse continuer sans interruption).
...
| 6.7 | Paiement (étape finale du `/goal` utilisateur, explicitement non prioritaire) — à faire ensemble avec l'utilisateur | ⏳ en attente utilisateur |
...
### 6.4c — online-exam-page.tsx : banque de questions, résultats, incidents ✅ (partiel, périmètre assumé)

Dernier point identifié dans l'audit initial encore sur données de démo. 3 nouveaux modèles (`ExamBankQuestion`, `ExamResult`, `ExamIncident`, migration additive), `/api/online-exams` étendu pour les exposer, `demoBankQuestions`/`demoStudentResults`/`demoIncidents` supprimés au profit des vraies données. Le bouton "Ajouter une question" n'avait litteralement aucun gestionnaire de clic — un vrai petit formulaire y a été branché. L'histogramme de distribution des notes (page résultats) était une liste de comptages fixes indépendante des résultats affichés — recalculé par vrais paliers depuis les scores réels.

**Périmètre volontairement exclu, documenté dans le code** : l'« Interface de passation » (le panneau qui simule un étudiant en train de passer l'examen — minuteur, navigation entre questions, carte de progression, confirmation de soumission) reste sur son jeu de questions fixe (`demoQuestions`). La brancher réellement demanderait un vrai système de passation en temps réel — session d'examen par étudiant, correction automatique des QCM, persistance des réponses en cours de frappe, déclenchement réel des incidents de surveillance (changement d'onglet, IP différente, etc.) — un chantier à part entière avec de vraies implications d'intégrité académique si mal fait, pas un simple remplacement de tableau démo comme les 3 autres sections de cette page. Décision prise sans consulter l'utilisateur au préalable (le `/goal` demande de ne pas s'arrêter pour demander), mais signalée explicitement ici et dans le code pour arbitrage ultérieur si souhaité.

**Reste à faire côté Chantier 6** : 6.5 (`RESEND_API_KEY`), 6.6 (Mobile Money), 6.7 (paiement — dernière étape du `/goal`, explicitement à faire avec l'utilisateur, pas en autonome), et le vrai système de passation d'examen en ligne (6.4c ci-dessus) si l'utilisateur le priorise. Follow-up mineur : aucune interface n'existe pour créer un `Advisor` (le conseil pédagogique ne peut afficher un conseiller que s'il a été créé directement en base pour l'instant).
