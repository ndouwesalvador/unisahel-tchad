...
**Nouveau point trouvé pendant cette passe, pas encore corrigé** : `student-detail.tsx` (la fiche détaillée d'un étudiant, ouverte depuis la liste des étudiants) est **entièrement** construite sur des données de démonstration figées (commentaire "Demo Student Data" en tête de fichier) — informations personnelles, notes, documents, paiements, stage. Le magasin d'état (`useAppStore`) contient déjà un `selectedStudentId` correctement renseigné par `students-list.tsx` au clic sur une ligne, mais `student-detail.tsx` ne le lit jamais : cliquer sur n'importe quel étudiant affiche systématiquement la même fiche fictive ("ABAKAR Adam Hassane"), quel que soit l'étudiant réellement sélectionné. C'est le plus sérieux des manques "sans démo" restants à ce stade — plus qu'un bouton isolé, une page entière déconnectée de la navigation réelle. Nécessite un point d'API agrégeant inscriptions + notes + documents + paiements + stage pour un `studentId` donné, comparable en ampleur à la refonte de `health-page.tsx` — prochain chantier.
