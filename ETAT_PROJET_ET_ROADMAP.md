...
>
> **Mise à jour du 2026-07-29 : Chantier 7 démarré — SaaS multi-tenant réel (Super Admin + provisioning d'institutions), 1ère moitié terminée.** L'utilisateur a demandé une architecture SaaS conforme à son projet sœur EduSahel (référence lue, jamais modifiée) : un Super Admin de plateforme crée les comptes d'institution, chaque institution crée ensuite ses propres comptes internes. `User.tenantId`/`AuditLog.tenantId` rendus nullables, route `/api/tenants` (liste + création transactionnelle institution+admin+mot de passe temporaire), script `bootstrap-super-admin.ts`, page `PlatformInstitutionsPage`. Vérifié de bout en bout contre le serveur de dev réel (connexion Super Admin → création d'institution → connexion du nouvel admin → accès à son tableau de bord). Détail en section [Chantier 7](#chantier-7--saas-multi-tenant--super-admin-plateforme--provisioning-dinstitutions-2026-07-29-️-1ère-moitié). **Reste à faire : la 2ème moitié** — chaque institution doit pouvoir créer ses propres comptes internes (enseignants, personnel) avec un vrai compte de connexion.
>
...
### Chantier 7 — SaaS multi-tenant : Super Admin plateforme & provisioning d'institutions (2026-07-29) ✅ (1ère moitié)
...
**Reste à faire (2ème moitié de la demande utilisateur)** : permettre à chaque institution de créer ses propres comptes internes (enseignants, personnel avec rôle) — aujourd'hui, la création d'un enseignant (`POST /api/teachers`) ne provisionne jamais de compte de connexion (`Teacher.userId` reste `null`), contrairement aux étudiants qui ont déjà `provisionStudentAccount`. Il n'existe par ailleurs aucune capacité générale « créer un utilisateur avec un rôle » utilisable par un `ADMIN_INSTITUTION` dans son propre tenant.

**Trouvé mais volontairement non traité dans cette passe** : `settings-page.tsx` (~1200 lignes, la page réellement affichée pour les vues "Paramètres" et — avant ce chantier — "Institution") est presque entièrement composée de données fictives (`defaultValue`/`defaultChecked` figés : nom d'admin, email, nom d'établissement, année académique...), avec zéro appel API. Ampleur suffisante pour mériter son propre chantier dédié plutôt qu'un correctif improvisé ici.

**Non traité non plus** : le flag `mustChangePassword`, posé correctement sur tout compte nouvellement créé (Super Admin bootstrap, admins d'institution via `/api/tenants`), n'est encore appliqué nulle part dans le flux de connexion — un utilisateur peut ignorer indéfiniment le mot de passe temporaire.
