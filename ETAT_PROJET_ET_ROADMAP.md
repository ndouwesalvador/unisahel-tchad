...
**Non modifié, volontairement** : `institution-page.tsx` a lui-même des onglets encore fictifs (Documents, Apparence, Abonnement, boutons "Ajouter" de l'onglet Structure) découverts pendant cette inspection croisée. Hors périmètre de la demande explicite ("résous settings-page.tsx") — → traité au Chantier 8.2 ci-dessous, sur validation de l'utilisateur.
...
#### 8.2 — institution-page.tsx : derniers onglets fictifs (Structure, Apparence, Documents, Abonnement) ✅ (2026-07-29, validé par l'utilisateur)

Suite directe du Chantier 8 : l'utilisateur a validé le traitement des onglets fictifs restants d'`institution-page.tsx`.

- **Onglet Structure — boutons "Ajouter faculté"/"Ajouter département" enfin fonctionnels** : `createFacultySchema` et `createDepartmentSchema` existaient dans `validations/api.ts` depuis le début mais **aucune route ne les utilisait** — les boutons n'avaient aucun gestionnaire de clic. Nouveau `POST /api/structure?type=faculty|department` (`withTenantAuth(['SUPER_ADMIN','ADMIN_INSTITUTION'])`, même règle que `/api/academic-years`), avec vérification que la faculté cible appartient bien au tenant avant de créer un département, et journal d'audit. Deux vrais dialogues de création dans l'onglet, avec invalidation du cache React Query. Au passage, `createDepartmentSchema` référençait des champs `email`/`phone` qui n'existent pas sur le modèle `Department` — retirés du schéma de validation avant qu'ils ne produisent la même erreur Prisma que celle des enseignants (7.2).
- **Onglet Apparence — couleurs réellement persistées** : les 3 pickers étaient de purs `useState` locaux, perdus au moindre changement d'onglet. Désormais initialisés depuis `TenantSettings.primaryColor/secondaryColor/accentColor` (champs déjà en base, déjà exposés par `SETTINGS_FIELDS` depuis le Chantier 8) et sauvegardés via `PUT /api/institution`. Le switch "Mode sombre" a été retiré : aucun `ThemeProvider` n'est monté dans l'app (`next-themes` n'est importé que par deux composants ui décoratifs), le toggle ne pouvait rien activer.
- **Onglet Informations — logo et cachet réellement enregistrés** : les zones "Clic pour televerser" ne faisaient rien (aucune infrastructure d'upload de fichiers n'existe dans le projet — pas de Blob/S3). Remplacées par des champs URL honnêtes (aperçu immédiat de l'image, mention explicite qu'il n'y a pas de téléversement de fichier), persistés dans `Tenant.logo`/`Tenant.stamp` — champs déjà présents dans le schéma, jamais écrits jusqu'ici, et que `documents/generate` lit déjà pour les PDF.
- **Onglet Documents — recentré sur le réel** : les sélecteurs de position QR/cachet (jamais lus par `templates.tsx`), la matrice de signataires fictive (5 rôles inventés) et l'en-tête/pied de page figés "Université de N'Djamena" sont retirés. Remplacés par : la liste des 5 types de documents réellement générés, le signataire réel (`rectorName`/`rectorTitle` du tenant), l'explication du système de vérification réel (`/verify` + QR), et un aperçu d'en-tête composé dynamiquement depuis les vraies données du tenant.
- **Onglet Abonnement — plan réel, plus de faux libre-service** : le plan "PRO 350 000 FCFA/mois" était figé avec de faux compteurs (2 847 étudiants, 18 Go de stockage...) et des boutons "Upgrader" morts. Affiche désormais le vrai `Tenant.subscriptionPlan` (celui que le Super Admin définit via `/api/tenants`), la date de fin d'abonnement réelle si définie, les vrais compteurs d'usage (bloc `stats` du Chantier 8), et un contact support par email à la place des boutons d'upgrade — la facturation en libre-service n'existe pas, le plan est géré par le Super Admin de la plateforme.
- `GET /api/institution` expose en plus `subscriptionPlan`/`subscriptionEnd` (lecture seule — la modification reste réservée au `PUT /api/tenants` du Super Admin) et `logo`/`stamp` dans `TENANT_FIELDS`.

**Vérifié** : `tsc`/`eslint`/`vitest` propres (0 erreur — après un `rm -rf .next`, le cache de build ayant été corrompu par les redémarrages successifs du serveur de dev). Script HTTP contre le serveur de dev réel : création d'institution → création de faculté puis de département via les nouvelles routes → rejet correct d'un département visant une faculté d'un autre tenant (404) → `GET /api/structure` reflétant la nouvelle hiérarchie → `PUT` couleurs+logo+cachet → re-lecture confirmant la persistance et l'exposition de `subscriptionPlan`. Données de test supprimées après coup.

### Chantier 9 — Candidature/Délibération : un vrai bug bloquant trouvé, plus balayage results-page.tsx (2026-07-30) ✅

L'utilisateur a signalé des données de démo dans les onglets Candidature et Délibération, et a demandé une vérification de la chaîne logique complète.

- **`candidature-page.tsx`** : en-tête (127 candidatures/mois, 68% admission) et 4 badges de tendance (+12%/+5%/+8%/-3%, toujours à la hausse) étaient des `useCountUp` sur des constantes. Remplacés par un vrai calcul mois-en-cours vs mois précédent à partir des `createdAt` réels. Le graphique "Candidatures par filière" (8 filières avec des comptes inventés) est maintenant une vraie répartition calculée sur les candidatures réelles. Le "Calendrier d'admission" (dates figées 2025) est remplacé par des faits réels : année académique en cours, date de première et dernière candidature reçue. **Bug plus sérieux** : le bouton "Soumettre la candidature" (formulaire inline + dialogue) ne faisait qu'un `setShowForm(false)` — aucun appel API, alors que `POST /api/candidature` existait déjà et fonctionnait. Câblé pour de vrai, avec un vrai sélecteur de filière (`useStructure()`, avant une liste de noms inventés) et une checklist de pièces jointes désormais transmise au serveur (`Admission.documents`, champ existant mais jamais utilisé). "Examiner"/"Contacter"/"Importer" (boutons morts) câblés sur de vraies actions.
- **`deliberation-page.tsx`** — **bug critique trouvé** : l'export du PV de délibération envoyait `tenantId: 'unknown'` et un objet `tenant` entièrement fictif ("Université de N'Djamena", "Pr. Mahamat Ali"...) à `/api/documents/generate`. Cette route vérifie strictement `sessionUser.tenantId === tenantId` et renvoie 403 sinon — **l'export PV était donc cassé à 100% pour tout utilisateur réel** (seul un SUPER_ADMIN aurait pu contourner la vérification, et ce rôle ne lance jamais de délibération). Corrigé avec le vrai `tenantId` de la session et les vraies données de l'institution (`useInstitution()`) et de l'année académique en cours. Les sélecteurs Filière/Niveau/Semestre, jamais branchés à aucune logique de filtrage réelle (confirmé dans `/api/deliberation` : les délibérations portent sur toute l'institution, sans granularité par filière/niveau/semestre), ont été retirés plutôt que laissés comme un faux contrôle. Colonnes Filière/Niveau de la liste des sessions (toujours "—") retirées pour la même raison.
- **`results-page.tsx`** — le balayage de la chaîne logique a révélé que le correctif antérieur (Chantier 6, tâche "Wire results-page.tsx to real Grade data") n'avait branché que 2 des 4 onglets. Trouvé et corrigé dans cette passe : 4 statistiques d'en-tête et une bannière dupliquée toujours sur `useCountUp(12/287/80/23, ...)` (remplacées par de vrais calculs sur `results`) ; un dialogue "Nouveau résultat" entier — avec sélecteur d'étudiant, notes par UE (UE301/302/303 inventées), calcul automatique de moyenne — dont le bouton "Enregistrer" n'avait **aucun gestionnaire `onClick`** (formulaire mort, ne pouvait jamais rien enregistrer) ; un toggle "Publier" sans aucune persistance ; l'onglet "Progression Académique" affichait un étudiant fictif figé ("MAHAMAT Ali", 150/180 crédits) **quel que soit ce qui était tapé dans la barre de recherche** (`searchProgression` n'était jamais utilisé), avec un graphique de moyenne par semestre et des badges de semestres complétés 100% inventés. Le dialogue mort et le toggle "Publier" ont été supprimés (ils dupliquent le vrai module Notes/Grades). L'onglet Progression cherche désormais réellement l'étudiant tapé dans `results` (données déjà réelles) et affiche ses vraies crédits/moyenne/décision de la session en cours ; le graphique par semestre inventé a été retiré (aucune donnée de regroupement par semestre n'existe côté API).
- **Trouvé mais volontairement non traité** : l'onglet "Statistiques" de `results-page.tsx` (distribution des moyennes, répartition par mention, comparaison annuelle 2021-2025, taux de réussite par faculté, comparaison hommes/femmes) reste entièrement fictif — corriger cela demanderait de nouvelles agrégations backend significatives (regroupement multi-années, démographie étudiante, réussite par faculté) qui n'existent nulle part dans l'API actuelle. Signalé pour un futur chantier dédié plutôt que traité superficiellement ici. Les filtres "Filière"/"Niveau" de l'onglet "Résultats par Session" de la même page sont également décoratifs (ne filtrent rien) — non corrigés dans cette passe.

**Vérifié** : `tsc`/`eslint`/`vitest` propres (0 erreur) sur les 4 fichiers touchés. Script HTTP contre le serveur de dev réel : création d'institution, création d'une candidature via `POST /api/candidature` avec `programId`/`academicYearId` réels et persistance confirmée du champ `documents`, puis **confirmation explicite que l'export PV via `POST /api/documents/generate` avec le vrai `tenantId` renvoie désormais 200 (PDF réel) au lieu du 403 systématique d'avant**. Données de test supprimées après coup.

### Chantier 10 — Performance : région Vercel/Neon désalignée + bundle JS de 2.3 Mo (2026-07-30) ✅

L'utilisateur a signalé une lenteur générale du site. Deux causes structurelles identifiées et corrigées, toutes deux quantifiées avant/après plutôt que corrigées à l'aveugle.

- **Région Vercel ≠ région base de données** : `vercel inspect` a confirmé que toutes les fonctions étaient déployées à `iad1` (Washington D.C., USA), alors que la base Neon (confirmée via `DATABASE_URL`) est à `eu-central-1` (Francfort, Allemagne). Chaque requête à la base traversait donc l'Atlantique deux fois — un aller-retour transatlantique typique ajoute 150-250ms **par requête**, et la plupart des pages en enchaînent plusieurs. Corrigé avec un `vercel.json` fixant `"regions": ["fra1"]` (Francfort, co-localisé avec Neon, et une latence raisonnable pour un public basé au Tchad/Afrique centrale via les points d'échange européens). Vérifié après redéploiement : `vercel inspect` confirme que toutes les fonctions tournent maintenant sur `[fra1]`.
- **Bundle JavaScript de 2.3 Mo chargé sur toute page** : `dashboard-shell.tsx` (le routeur applicatif, basé sur un switch Zustand plutôt que sur de vraies routes Next.js) importait statiquement ses ~40 vues (`teachers-page`, `results-page` à 1400+ lignes, `payments-page`, `statistics-page`...) — c'est-à-dire que le code de chaque module de l'application, utilisé ou non par l'utilisateur courant, était fusionné dans un seul chunk JS de **2.3 Mo** chargé au premier affichage du tableau de bord. Corrigé en convertissant les 38 imports de vues en `next/dynamic()` avec un fallback de chargement (`ViewLoading`), pour que seul le code de la vue réellement affichée soit téléchargé. **Mesuré avant/après sur un build de production réel** : le plus gros chunk passe de 2.3 Mo à 420 Ko (−82%), avec désormais de nombreux petits chunks chargés à la demande au lieu d'un seul bloc monolithique. Vérifié en conditions réelles dans le navigateur (session authentifiée réelle, via script JS exécuté dans la page puisque les clics ne fonctionnent pas dans cet environnement d'exécution) : `dashboard-shell` et `dashboard-home` se chargent bien comme des fichiers séparés, aucune erreur console, le tableau de bord s'affiche correctement avec les vraies données de l'institution de test.

**Non exploré dans cette passe, pistes possibles si la lenteur persiste** : la requête imbriquée sur 6 niveaux de `/api/structure` (faculté → département → programme → niveau → semestre → UE → élément de cours) pourrait devenir coûteuse à mesure que les données grandissent, bien qu'elle bénéficie déjà d'un cache React Query de 5 minutes côté client ; absence d'index composites explicites au-delà de `tenantId` sur certains modèles à forte volumétrie (Grade, Payment).

**Vérifié** : `tsc`/`eslint`/`vitest` propres (0 erreur). Build de production réel exécuté deux fois (avant/après) pour mesurer la taille des chunks. Déploiement réel sur Vercel avec confirmation de la région via `vercel inspect`. Vérification navigateur en session authentifiée réelle (compte de test créé puis supprimé).
