...
>
> **Mise à jour du 2026-07-26 : Chantier 6 (« mode réel ») très avancé.** L'utilisateur a demandé de sortir du mode démo : audit complet fait (voir [Chantier 6](#chantier-6--sortir-du-mode-démo-vers-un-vrai-saas)), boutons de connexion démo retirés de la production, tableau de bord principal branché sur de vraies données de bout en bout, **inscription de nouveaux établissements ajoutée** (le plus gros manque structurel — ce n'était auparavant qu'une app mono-tenant), et **les 6 modules à backend existant (scholarships, hr, internships, attendance, room-booking, results) branchés sur de vraies données**. Reste : 4 modules sans aucun modèle Prisma (`advising`, `library`, `transport`, `notification-panel` — pas encore scopé avec l'utilisateur), `RESEND_API_KEY` à fournir, Mobile Money réel (attente utilisateur).
...
| 6.4b | `advising-page.tsx`, `library-page.tsx`, `transport-page.tsx`, `notification-panel.tsx` — aucun modèle Prisma du tout, nécessitent de vraies migrations. Non commencé, à prioriser avec l'utilisateur | ⏳ à faire |
...
**Reste à faire** : 6.4b (`advising`, `library`, `transport`, `notification-panel` — nécessitent de nouveaux modèles Prisma, pas encore scopé avec l'utilisateur), 6.5, 6.6.
