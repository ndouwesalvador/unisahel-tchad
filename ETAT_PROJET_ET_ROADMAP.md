...
**Reste à faire côté Chantier 6** : 6.5 (`RESEND_API_KEY`), 6.6 (Mobile Money), 6.7 (paiement — dernière étape du `/goal`, explicitement à faire avec l'utilisateur, pas en autonome). Follow-ups mineurs identifiés en cours de route, non bloquants : aucune interface n'existe pour créer un `Advisor` (le conseil pédagogique ne peut afficher un conseiller que s'il a été créé directement en base pour l'instant) ; `online-exam-page.tsx` garde encore des données de démo pour la banque de questions/résultats/incidents de surveillance (identifié dans l'audit initial, jamais traité, toujours hors scope).
