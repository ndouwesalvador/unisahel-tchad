...
>
> **Mise à jour du 2026-07-26 : Chantier 6 (« mode réel ») en bonne voie.** L'utilisateur a demandé de sortir du mode démo : audit complet fait (voir [Chantier 6](#chantier-6--sortir-du-mode-démo-vers-un-vrai-saas)), boutons de connexion démo retirés de la production, tableau de bord principal branché sur de vraies données de bout en bout, et **inscription de nouveaux établissements ajoutée** (le plus gros manque structurel — ce n'était auparavant qu'une app mono-tenant). Reste : ~9 modules encore sur données de démo, `RESEND_API_KEY` à fournir, Mobile Money réel (attente utilisateur).
...
**Reste hors périmètre** (aucun modèle Prisma, nécessiterait migrations + API + frontend — scope "Groupe 3", non demandé) : `transport`, `library`, `results`, `advising`, `notification-panel`.
...
**Reste à faire** : voir tableau Chantier 6 plus haut (6.4-6.6).
