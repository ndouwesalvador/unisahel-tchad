...
> extends React.SVGProps<SVGSVGElement> {
  ...
  stagger?: boolean
  pulse?: boolean
  /** Crop the map to a lat/lng bounding box instead of rendering the whole world. */
  region?: MapRegion
  /** Restrict rendered dots to specific country codes. */
  countries?: string[]
...
export function DottedMap<M extends Marker = Marker>({
  ...
  const { points, processedMarkers } = React.useMemo(() => {
    const map = createMap({ width, height, mapSamples, region, countries })
    return { points: map.points, processedMarkers: map.addMarkers(markers) }
  }, [width, height, mapSamples, region, countries, markers])
